window.onload=function(){
	...
	function praiseBox(box,el){
		...
		/*它是用getElementsByClassName('praises-total') 
		 ...
		 var oldTotal=parseInt(praiseElement.getAttribute('total'));//自定义的属性获取用getAttribute()
		 var txt=el.innerHTML;//el是a标签,el.innerHTML可能是'赞'，也可能是'取消赞''
		 var newTotal;//定义一个变量，如果txt是'赞'，就总数+1，如果txt是'取消赞'，就总数-1

		 if (txt=='赞') {
		 	newTotal=oldTotal+1;
		 	praiseElement.setAttribute('total',newTotal);//使用setAttribute()来设置元素的属性,要更新total的值，不然，会在原来的tota上减1
		 	...
		 	el.innerHTML='赞';//当点击赞的时候，这个位置就是取消赞
		 }
     ...
     */
      function praiseReply(el){
      	...
      	if (myPraise==0) {//如果点赞，就做出改变
      		...
      		el.style.display=(newTotal==0)?'':'inline-block';//如果有点赞，就不隐藏，没有的话，就隐藏。
      }
       /**
     * 操作留言
     * @param el 点击的元素
